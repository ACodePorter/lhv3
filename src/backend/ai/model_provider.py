...
class DeepSeekPriceModel(AIModel):
    ...
    def _build_prompt(self, history: pd.DataFrame, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        header_parts.append(
            ...
            "并结合账户状态和历史交易记录，综合判断此刻应该执行“买入(BUY) / 卖出(SELL) / 观望(HOLD)”中的哪一种操作，"
            "以及本次操作建议使用的仓位比例（0 到 1 之间的小数）。"
        )
        header_parts.append(
            "输出格式要求：先给出一个数字表示预测收盘价，然后用一句话解释主要原因，"
            "在同一行最后明确给出操作指令和仓位比例，格式为“操作: BUY / SELL / HOLD, 仓位: x”，"
            "其中 x 为 0 到 1 之间的小数（例如 0.25 表示使用 25% 可用资金），"
            "例如：123.45, 近期放量上涨、趋势保持强势，操作: BUY, 仓位: 0.5。"
        )
